...
describe PricePlanComparatorController do
  ...
  describe '/price-plan/calculate-last-week/{meter_id}' do
    let(:now) { Time.now.freeze }

    it 'should return no match if there is no meter with that meter id' do
      get '/price-plans/calculate-last-week/meter-1000'
      expect(last_response.status).to eq(404)
    end
    
    it 'returns 200 for a meter id' do
      get '/price-plans/calculate-last-week/meter-0'
      readings = [
        { 'time' => '2018-01-01T00:00:00.000Z', 'reading' => 15.0 },
        { 'time' => '2018-01-01T01:00:00.000Z', 'reading' => 5.0 }
      ]
    ...
    it 'returns last week\'s cost for a meter id' do
      get '/price-plans/calculate-last-week/meter-0'
      readings = [
        { 'time' => '2018-01-01T00:00:00.000Z', 'reading' => 15.0 },
        { 'time' => '2018-01-01T01:00:00.000Z', 'reading' => 5.0 }
